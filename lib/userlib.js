        ...
        if (err) {
            ...
            thislib.validate(userInfo, function (isInvalid, msg) {
                ...
                var city = new locationModel({
                    namme: userInfo.location.terms[0],
                    type: "City"
                });
                //city.validate().then(function(response){
                city.save(function (err, done) {
                    if (err) throw err;
                });
                //});
                var state = new locationModel({
                    name: userInfo.location.terms[1],
                    type: 'State'
                });
                state.validate().then(function (response) {
                ...
                var country = new locationModel({
                    name: userInfo.location.terms[2],
                    type: 'Country'
                });
                country.validate().then(function (response) {
                    country.save();
                });
